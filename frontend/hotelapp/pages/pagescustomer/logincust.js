      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          if (role === 'tamu') {
            ...
            alert('Failed to login: sampeyan is not customer');
          }
        } else {
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleLogin}>
            ...
            <p className="flex flex-col items-center justify-center mt-10 text-center text-gray-500 text-md">
              ...
              <a href="/pagescustomer/register" className="text-indigo-400 no-underline transition duration-300 ease-in cursor-pointer hover:text-blue-500 hover:underline">Sign
                up</a>
            </p>
          </form>
        </div>
