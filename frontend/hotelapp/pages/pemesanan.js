...
export default function Pemesanan() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8080/pemesanan', {
        headers: {
  ...
  pemesanan.nama_tamu.toLowerCase().includes(searchTerm.toLowerCase()) ||
  pemesanan.tgl_check_in.includes(searchTerm)
);
...
const handleSearch = (e) => {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const pemesananId = selectedPemesananId; // Get the selected pemesanan ID
      ...
      await axios.put(
        `http://localhost:8080/pemesanan/updatestatus/${pemesananId}`,
        updatedData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      fetchData(); // Refresh the pemesanan data
  
      handleModalToggle(selectedPemesananId); // Close the modal
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleStatusPemesananChange = (e) => {
    setStatusPemesanan(e.target.value);
  };
  ...
  const handleModalToggle = (pemesananId) => {
    setSelectedPemesananId(
      pemesananId === selectedPemesananId ? null : pemesananId
    );
  };
  ...
  const handleAddSubmit = async (e) => {
    ...
    const formData = {
      // nomor_pemesanan: e.target.elements.nomor_pemesanan.value,
      ...
      nomor_kamar: e.target.elements.nomor_kamar.value,
      status_pemesanan: e.target.elements.status_pemesanan.value,
      nama_user: e.target.elements.nama_user.value,
      details_of_pemesanan: [
        { harga: e.target.elements.harga.value }
      ]
    };
    ...
    try {
      ...
      await axios.post("http://localhost:8080/pemesanan/add", formData, {
        headers: {
          Authorization: `Bearer ${token}`
        },
      });
  ...
  <div className="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl dark:bg-gray-900 sm:my-8 sm:w-full sm:max-w-sm sm:p-6 sm:align-middle">
    ...
    >
      ...
      <label htmlFor="harga" className="block mt-3">
    Harga
    <input
      type="text"
      name="harga"
                        ...
                        <tr key={pemesanan.id}>
                          ...
                          <td className="px-10 py-4 text-sm whitespace-nowrap">
                            <div className="flex items-center gap-x-12">
                              ...
                              {/* <button
                                // onClick={() => deleteTipeKamar(pemesanan.id)}
                              ...
                              <button
                                ...
                                onClick={() => handleModalToggle(pemesanan.id)}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  fill="none"
...
<label htmlFor="status_pemesanan" className="text-sm text-gray-700 dark:text-gray-200">
  Status Pemesanan
</label>
<label
        className="block mt-3"
...
<select
  ...
  value={
    selectedPemesananId &&
    pemesananData.find((pemesanan) => pemesanan.id === selectedPemesananId)?.status_pemesanan
  }
  onChange={handleStatusPemesananChange}
